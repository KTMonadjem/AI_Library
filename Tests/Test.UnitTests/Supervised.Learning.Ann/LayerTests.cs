...
public class LayerTests
{
    ...
    public void CreateWithRandomWeights_Should_CreateWithRandomWeightsCorrectly(int numberOfNeurons,
        int numberOfWeights, double minWeight, double maxWeight)
    {
        var layer = Layer.CreateWithRandomWeights(numberOfNeurons, numberOfWeights, minWeight, maxWeight,
            _activationFunction);
        ...
        foreach (var neuron in layer.Neurons)
        {
            ...
            foreach (var weight in neuron.Weights) weight.Should().BeInRange(minWeight, maxWeight);
        }
    ...
    {
        Action act = () =>
            Layer.CreateWithRandomWeights(numberOfNeurons, _numberOfWeights, _minWeight, _maxWeight,
                _activationFunction);
    ...
    {
        Action act = () =>
            Layer.CreateWithRandomWeights(_numberOfNeurons, numberOfWeights, _minWeight, _maxWeight,
                _activationFunction);
    ...
    [TestCase(5, 0)]
    [TestCase(0.1, -0.1)]
    public void CreateWithRandomWeights_Should_ThrowException_When_MinWeightLessThanMaxWeight(double minWeight,
        double maxWeight)
    {
        Action act = () =>
            Layer.CreateWithRandomWeights(_numberOfNeurons, _numberOfWeights, minWeight, maxWeight,
                _activationFunction);
    ...
    [TestCaseSource(nameof(LayerDataSources))]
    public void BuildWeights_Should_CorrectlyBuildNeuronWeights(double[,] weightInputs, int numberOfNeurons,
        int numberOfWeights)
    {
        var weightsMatrix = M.DenseOfArray(weightInputs);
    ...
    {
        var layer = Layer
            .CreateWithRandomWeights(_numberOfNeurons, _numberOfWeights, _minWeight, _maxWeight, _activationFunction)
            .BuildWeights();
    ...
    public void AddInputs_Should_Succeed_When_InputsAreValid()
    {
        ...
        for (var i = 0; i < _numberOfNeurons; i++) inputs[i] = i;
        var layer = Layer
            .CreateWithRandomWeights(_numberOfNeurons, _numberOfNeurons, _minWeight, _maxWeight, _activationFunction)
            .BuildWeights()
            .AddInputs(V.DenseOfArray(inputs));

        foreach (var neuron in layer.Neurons) neuron.Inputs!.Should().BeEquivalentTo(inputs);
    }
    ...
    public void AddParents_Should_Succeed_When_ValidParents()
    {
        ...
        foreach (var neuron in layer.Neurons) neuron.Parents.Should().BeEquivalentTo(parents);
    }
    ...
    public void AddParentLayer_Should_Succeed_When_ValidParents()
    {
        ...
        foreach (var neuron in layer.Neurons) neuron.Parents.Should().BeEquivalentTo(parents.Neurons);
    }

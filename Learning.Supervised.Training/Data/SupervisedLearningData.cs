...
public class SupervisedLearningData : IDataSet
{
    ...
    {
        if (_inputs.ColumnCount <= column) throw new ArgumentOutOfRangeException(nameof(column));
        if (_outputs.ColumnCount <= column) throw new ArgumentOutOfRangeException(nameof(column));
        return Matrix<double>.Build.DenseOfColumns(new List<Vector<double>>
            { _inputs.Column(column), _outputs.Column(column) });
    }
}